    ...
    return new Promise((resolve) => {
      try {
        ...
        onceFn(rotationVectorId, (data: Record<string, number>) => {
          ...
          resolve(this.normalizeHeading({ x, y, z, w }));
        });
      } catch (_error) {
        resolve({
          magHeading: 0,
...
async function watchNative(kind, args, callback, errorHandler) {
  const watchId = nextWatchId++;
  // Register before crossing the bridge so an immediate native fix is not lost.
  const listener = DeviceEventEmitter.addListener('ExpoHarmonyLocationWatch', (event) => {
    if (event.watchId !== watchId) return;
    if (event.error !== undefined) {
      if (typeof errorHandler === 'function') errorHandler(String(event.error));
    } else if (kind === 'Position' && event.location) {
      callback(normalizeLocationObject(event.location));
    } else if (kind === 'Heading' && event.heading) {
      callback(normalizeHeadingObject(event.heading));
    }
  });
  try {
    await invokeNative('startWatch' + kind, 'watch' + kind + 'Async', ...args, { watchId });
  } catch (error) {
    listener.remove();
    if (typeof errorHandler === 'function') errorHandler(error.message);
    throw error;
  }
  return createSubscription(() => {
    listener.remove();
    void invokeNative('stopWatch' + kind, 'LocationSubscription.remove', watchId)
...
module.exports = {
  ...
  async watchPositionAsync(options, callback, errorHandler) {
    ...
    return watchNative('Position', [options ?? {}], callback, errorHandler);
  },
  async watchHeadingAsync(callback) {
    ...
    return watchNative('Heading', [], callback);
  },
