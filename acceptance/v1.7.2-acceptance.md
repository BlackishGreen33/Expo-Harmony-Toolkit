...
- `evidenceSource.bundle/debugBuild` 固定标记为 `automated`
- `evidenceSource.device` 固定标记为 `manual-doc`
- `evidenceSource.release` 固定标记为 `none`
...
- [support-matrix](../docs/support-matrix.md) 已更新为新的 yellow subset
- [official-native-capabilities-sample](../docs/official-native-capabilities-sample.md) 已改成四条 route 的收口版 walkthrough
...
- 真机与 release 证据仍需后续验收补齐；当前 `device=yes` 仅代表已有人工记录，不代表自动化设备验证
