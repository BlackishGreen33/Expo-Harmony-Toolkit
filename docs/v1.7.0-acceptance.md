...
- `documentDirectory` 成功显示真实 Harmony sandbox 路径
- 页面提供独立的 `Write file`、`Read file`、`Inspect file`、`List sandbox directory`、`Delete file` 与 `Run full write/read/delete flow` 按钮，便于逐步验收
- 点击 `Run full write/read/delete flow` 后，页面返回
  `Functional flow OK. existsAfterWrite=true contents=expo-harmony-functional existsAfterDelete=false`
- 已在模拟器上完成 UTF-8 写入、读回、删除闭环
- 当前结论应更新为 `runtimeMode=adapter`，但仍保留在 `preview`，因为 release 证据尚未补齐

### Image Picker

- `/image-picker` route 成功打开
- 页面提供独立的 `Request media permission`、`Check media permission`、`Request camera permission`、`Check camera permission`、`Launch image library`、`Launch camera capture`、`Inspect latest picker result` 与两条完整 flow，便于逐步验收
- `Check media permission` 返回 `Media permission OK. status=granted granted=true canAskAgain=true access=all`
- `Launch image library` 可拉起系统图库 UI，页面可观察到 `安全访问图库`、`所有图片`、`所有相册`
- 在系统图库右上角点击 `完成` 后，页面返回 `Image library canceled. canceled=true assets=0`
- `Launch camera capture` 当前通过图库拍照入口拉起系统 UI；在退出后，页面返回 `Camera capture canceled. canceled=true assets=0`
- `Inspect latest picker result` 能把最近一次取消结果回显为 `Inspect OK. source=camera capture canceled=true assets=0`
- 当前模拟器图库没有现成图片，本轮尚未补齐“真实 asset 返回”与“显式拒权”两类证据，因此 `expo-image-picker` 仍保持 `preview/runtimeMode=adapter`
