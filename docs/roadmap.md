...
状态：`v1.11.0` 是未发布的 burn-down ledger checkpoint；`v1.11.1` 是第一个公开 `v1.11.x`，发布到 `latest`，只收口 sidecar drift 工具链行为；`v1.11.2` 已发布到 `latest`，完成非实机 closeout 与 ccnubox signed simulator app-shell gate；`v1.11.3` 已发布到 `latest`，完成 v2 readiness 非实机 gate / fallback / exception 台账且不提升任何 capability 到 `verified`；`v1.11.4` 进行 publication-state reconciliation，并保持相同的 verified 边界。
...
- 新增 `acceptance/v1.11.4-acceptance.md` 记录 publication-state reconciliation 与本地/发布证据
