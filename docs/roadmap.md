...
状态：`v1.11.0` 是未发布的 burn-down ledger checkpoint；`v1.11.1` 是第一个公开 `v1.11.x`，发布到 `latest`，只收口 sidecar drift 工具链行为；`v1.11.2` 准备非实机 burn-down 台账与降级策略，不提升任何 capability 到 `verified`。
...
- 新增 `acceptance/v1.11.2-acceptance.md` 记录非实机 burn-down 台账、bare release gap 与 third-party fallback 策略
